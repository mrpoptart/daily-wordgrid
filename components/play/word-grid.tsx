...
export function WordGrid({ board, boardDate }: WordGridProps) {
  ...
  const { wordsWithinTime, wordsAfterTime, scoreWithinTime, scoreAfterTime } = useMemo(() => {
    ...
    words.forEach(w => {
      // If we don't have boardStartedAt, assume within time (or handle gracefully)
      // If timestamp is missing (optimistic), use current time?
      // Actually, if we just added it, we want to categorize it correctly.
      // But we have boardStartedAt.
